...
public class sos_module2_adapter extends ArrayAdapter {
	Context context;
	int resource;
	public ArrayList<ArrayList<String>> data;
	public sos_module2_adapter(Context context, int resource,String[]  place) {
		super(context, resource);
		this.context = context;
		this.resource = resource;
		myDB mydb=new myDB(context);
		mydb.Connect();
		data=mydb.selectseven(place);
		mydb.closedb();
		/*
		if(data.size()==0){
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		View row = inflater.inflate(R.layout.sos_module2_item, parent, false);//youtubethumbnailview
